...
var diff_match_patch = function() {
   ...
   */
  diff_match_patch.prototype.patch_apply = function(patches, text, step_callback = null) {
    if (patches.length == 0) {
      return [text, []];
    }
    ...
    for (var x = 0; x < patches.length; x++) {
      ...
      if (start_loc == -1) {
        ...
        if (text1 == text2) {
          ...
          // Step callback
          const replacement = this.diff_text2(patches[x].diffs);
          callback(
            start_loc,
            text1.length - 2 * this.Patch_Margin,
            replacement.substring(this.Patch_Margin, replacement.length - this.Patch_Margin),
          );
        } else {
